class Mavuika {
  ...
  class citlali {
    ...
    calculate_basic_dmg(dmg_rate, status) {
      ...
      if (this.reaction_coeff > 0)
      {
        attckRate = status[2] * dmg_rate[2][0] + status[4] * dmg_rate[4][0] + calculate_weapon_basedmg(this.react_attack_count, status, this.weapon_rank, this.base_dmgbuff);
        basicDmg = attckRate * this.reaction_coeff * (1 + this.reaction_bonus + 2.78 * status[2] / (status[2] + 1400))
                  + status[2] * dmg_rate[2][1] + status[4] * dmg_rate[4][1] + calculate_weapon_basedmg(this.nonreact_attack_count, status, this.weapon_rank, this.base_dmgbuff);
      }
      else
      {
        if (attack_method_index == 3 || attack_method_index == 4)
        {
          basicDmg = status[2] * (dmg_rate[2][0] + dmg_rate[2][1]) + status[4] * (dmg_rate[4][0] + dmg_rate[4][1]) + calculate_weapon_basedmg(this.react_attack_count + this.nonreact_attack_count, status, this.weapon_rank, this.base_dmgbuff);
        }
        else
        {
          basicDmg = status[4] * (dmg_rate[4][0] + dmg_rate[4][1]) + calculate_weapon_basedmg(this.react_attack_count + this.nonreact_attack_count, status, this.weapon_rank, this.base_dmgbuff);
  ...
  class sethos {
    ...
    calculate_basic_dmg(dmg_rate, status) {
      if (this.reaction_coeff > 0)
      {
        const attckRate = status[2] * dmg_rate[2] + status[4] * dmg_rate[4] + calculate_weapon_basedmg(this.attack_hit_count, status, this.weapon_rank, this.base_dmgbuff);
        let basicDmg = (attckRate + this.aggcount * this.reaction_coeff * (this.parameter[1]) * (1 + this.reaction_bonus + 5 * status[2] / (status[2] + 1200)));
        return basicDmg;
      }
      else
  ...
  class kachina {
    ...
    calculate_basic_dmg(dmg_rate, status) {
      let basicDmg;
      basicDmg = dmg_rate[1] * status[1] + calculate_weapon_basedmg(this.attack_hit_count, status, this.weapon_rank, this.base_dmgbuff);
      return basicDmg;
    }
