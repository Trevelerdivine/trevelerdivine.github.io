  ...
  class sigewinne {
    ...
    async dmg_rate_data() {
      ...
      // JSON データを取得
      const response = await fetch("../data/character/char_data/Sigewinne.json");
      const data = await response.json();
      ...
      if (attack_method == 16)
      {
        const attack_count1 = parseInt(document.getElementById("sigwinne_skill_count1").value);
        const attack_count2 = parseInt(document.getElementById("sigwinne_skill_count2").value);
        const attack_count3 = parseInt(document.getElementById("sigwinne_skill_count3").value);
        const react_count1 = parseInt(document.getElementById("sigwinne_react_count1").value);
        const react_count2 = parseInt(document.getElementById("sigwinne_react_count2").value);
        const react_count3 = parseInt(document.getElementById("sigwinne_react_count3").value);
  
        this.react_attack_count = [react_count1, react_count2, react_count3];
        this.nonreact_attack_count = [attack_count1 - react_count1, attack_count2 - react_count2, attack_count3 - react_count3];
        
        
        const dmg_hp_rate = parseFloat(data["元素スキル"]["詳細"][0]["数値"][this.parameter[3]]);
        const elm_react_dmgrate1 = react_count1 * dmg_hp_rate;
        const elm_react_dmgrate2 = react_count2 * dmg_hp_rate;
        const elm_react_dmgrate3 = react_count3 * dmg_hp_rate;
        elm_react_dmgrate = [elm_react_dmgrate1, elm_react_dmgrate2, elm_react_dmgrate3];
  
        const elm_nonreact_dmgrate1 = (attack_count1 - react_count1) * dmg_hp_rate;
        const elm_nonreact_dmgrate2 = (attack_count2 - react_count2) * dmg_hp_rate;
        const elm_nonreact_dmgrate3 = (attack_count3 - react_count3) * dmg_hp_rate;
        elm_nonreact_dmgrate = [elm_nonreact_dmgrate1, elm_nonreact_dmgrate2, elm_nonreact_dmgrate3];
    ...
    calculate_basic_dmg(dmg_rate, status) {
      ...
      if (this.reaction_coeff > 0)
      {
        if (attack_method == 16)
        {
          let ReactRate = (status[0] * dmg_rate[0][0][0] + calculate_weapon_basedmg(this.react_attack_count[0], status, WeaponConstellations, this.base_dmgbuff)) * (1.00 + status[7])
                        + (status[0] * dmg_rate[0][0][1] + calculate_weapon_basedmg(this.react_attack_count[1], status, WeaponConstellations, this.base_dmgbuff)) * (1.05 + status[7])
                        + (status[0] * dmg_rate[0][0][2] + calculate_weapon_basedmg(this.react_attack_count[2], status, WeaponConstellations, this.base_dmgbuff)) * (1.10 + status[7]);
  
          let nonReactRate = (status[0] * dmg_rate[0][1][0] + calculate_weapon_basedmg(this.nonreact_attack_count[0], status, WeaponConstellations, this.base_dmgbuff)) * (1.00 + status[7])
                           + (status[0] * dmg_rate[0][1][1] + calculate_weapon_basedmg(this.nonreact_attack_count[1], status, WeaponConstellations, this.base_dmgbuff)) * (1.05 + status[7])
                           + (status[0] * dmg_rate[0][1][2] + calculate_weapon_basedmg(this.nonreact_attack_count[2], status, WeaponConstellations, this.base_dmgbuff)) * (1.10 + status[7])
      ...
      else
      {
        if (attack_method == 16)
        {
          attckRate = (status[0] * (dmg_rate[0][0][0] + dmg_rate[0][1][0]) + calculate_weapon_basedmg(this.react_attack_count[0] + this.nonreact_attack_count[0], status, WeaponConstellations, this.base_dmgbuff)) * (1 + status[7])
                    + (status[0] * (dmg_rate[0][0][1] + dmg_rate[0][1][1]) + calculate_weapon_basedmg(this.react_attack_count[1] + this.nonreact_attack_count[1], status, WeaponConstellations, this.base_dmgbuff)) * (1.05 + status[7])
                    + (status[0] * (dmg_rate[0][0][2] + dmg_rate[0][1][2]) + calculate_weapon_basedmg(this.react_attack_count[2] + this.nonreact_attack_count[2], status, WeaponConstellations, this.base_dmgbuff)) * (1.10 + status[7]);
    ...
    calculate_char_debuff() {
      ...
      let char_debuff = [debuff_effect,0,0];
      return char_debuff;
    }
  }
  ...
  class sethos {
    ...
    async dmg_rate_data() {  
      ...
      if (reaction_check.checked)
      {
        this.aggcount = parseInt(document.getElementById("sethos_agg_count").value);
        this.reaction_coeff = 1.15
      }
      ...
      if (CharConstellations > 2)
      {
        const sethos_fourthConste_check = document.getElementById("traitCheckbox4");
        if (sethos_fourthConste_check.checked)
        {
          this.fourth_conste_buff = 80;
        }
      } 
      ...
      if (attack_method == 6) {
        const attack_count1 = parseInt(document.getElementById("sethos_attack_count1").value);
        const attack_count2 = parseInt(document.getElementById("sethos_attack_count2").value);
        const attack_count3 = parseInt(document.getElementById("sethos_attack_count3").value);
        this.attack_hit_count = attack_count1 + attack_count2 * 2 + attack_count3;
        const Burst_Buff_Rate = parseFloat(data["元素爆発"]["詳細"][0]["数値"][this.parameter[3]]);
        ...
        dmg_elm_rate = Burst_Buff_Rate * this.attack_hit_count;
        ...
        dmg_attack_rate = parseFloat(data["重撃"]["詳細"][1]["数値"][this.parameter[3]])  * this.attack_hit_count;
        dmg_elm_rate = parseFloat(data["重撃"]["詳細"][2]["数値"][this.parameter[3]]) * this.attack_hit_count + 7 * attack_count1; 
    ...
    calculate_basic_dmg(dmg_rate, status) {
      if (this.reaction_coeff > 0)
      {
        const attckRate = status[2] * dmg_rate[2] + status[4] * dmg_rate[4] + calculate_weapon_basedmg(this.attack_hit_count, status, WeaponConstellations, this.base_dmgbuff);
        let basicDmg = (attckRate + this.aggcount * this.reaction_coeff * (this.parameter[1]) * (1 + this.reaction_bonus + 5 * status[2] / (status[2] + 1200)));
        return basicDmg;
      }
      else
  ...
  class kinich {
    constructor(base_status_array, parameter) {
      ...
      this.attack_count2 = 0;
      this.attack_count3 = 0;
      const fix_basedmg_buff = parseFloat(document.getElementById("fix_basedmg_buff").value) || 0;
      const dynamic_basedmg_buff = parseFloat(document.getElementById("dynamic_basedmg_buff").value) || 0;
      this.base_dmgbuff = fix_basedmg_buff + dynamic_basedmg_buff;
    }
    ...
    async dmg_rate_data() {
      ...
      // JSON データを取得
      const response = await fetch("../data/character/char_data/kinich.json");
      const data = await response.json();
    
      // 攻撃方法に応じてダメージ率を計算
      let dmg_rate;
      let dmg_attck_rate = [0,0,0];
      ...
      if (attack_method == 16) {
        ...
        dmg_attck_rate[0] = parseFloat(data["元素スキル"]["詳細"][0]["数値"][this.parameter[3]]) * this.attack_count1;
        dmg_attck_rate[1] = parseFloat(data["元素スキル"]["詳細"][1]["数値"][this.parameter[3]]) * this.attack_count2
                          + 3.2 * (attack_count3 + attack_count4 * 2);
        ...
        this.attack_count1 = attack_count1 + attack_count2;
        
        dmg_rate = [0, 0, 0, 0, dmg_attck_rate1, 0, 0];
      }
    ...
    calculate_basic_dmg(dmg_rate, status) {
      if (this.reaction_coeff > 0)
      {
        if (attack_method == 16)
        { 
          const agg_bonus = this.reaction_coeff * this.parameter[1] * (1 + 5 * status[2] / (status[2] + 1200));
          const critical_coeff = (1 + status[5] * (this.first_conste_buff + status[6])) / (1 + status[5] * status[6]);
  
          const basicDmg1 = status[4] * dmg_rate[4][0] + calculate_weapon_basedmg(this.attack_count1, status, WeaponConstellations, this.base_dmgbuff)
                          + this.agg_count1 * agg_bonus;
          const basicDmg2 = (status[4] * dmg_rate[4][1] + calculate_weapon_basedmg(this.attack_count2, status, WeaponConstellations, this.base_dmgbuff)
                          + this.agg_count2 * agg_bonus) * (1 + status[7] + this.second_conste_buff) / (1 + status[7]);
          const basicDmg3 = status[4] * dmg_rate[4][2] + calculate_weapon_basedmg(this.attack_count3, status, WeaponConstellations, this.base_dmgbuff)
        ...
        { 
          const total_rate = status[4] * dmg_rate[4] + calculate_weapon_basedmg(this.attack_count1, status, WeaponConstellations, this.base_dmgbuff);
      ...
      else
      {
        if (attack_method == 16)
        {
          ...
          const basicDmg1 = status[4] * dmg_rate[4][0] + calculate_weapon_basedmg(this.attack_count1, status, WeaponConstellations, this.base_dmgbuff);
          const basicDmg2 = (status[4] * dmg_rate[4][1] + calculate_weapon_basedmg(this.attack_count2, status, WeaponConstellations, this.base_dmgbuff))
                          * (1 + status[7] + this.second_conste_buff) / (1 + status[7]);
          const basicDmg3 = status[4] * dmg_rate[4][2] + calculate_weapon_basedmg(this.attack_count3, status, WeaponConstellations, this.base_dmgbuff);
          const basicDmg = basicDmg1 + (basicDmg2 + basicDmg3) * critical_coeff;
          return basicDmg;
        }
  ...
  class collei {
    ...
    async dmg_rate_data() {      
      ...
      if (reaction_check.checked)
      {
        this.aggcount = parseInt(document.getElementById("collei_agg_count").value);
        this.reaction_coeff = 1.25
      }
  
      // JSON データを取得
      const response = await fetch("../data/character/char_data/collei.json");
      const data = await response.json();
      ...
      if (attack_method == 16) {
        const attack_count1 = parseInt(document.getElementById("collei_skill_count1").value);
        const attack_count2 = parseInt(document.getElementById("collei_skill_count2").value);
        this.attack_hit_count = attack_count1 + attack_count2;
        dmg_attck_rate = parseFloat(data["元素スキル"]["詳細"][0]["数値"][this.parameter[3]]) * attack_count1
                       + 0.4 * attack_count2;
        dmg_rate = [0, 0, 0, 0, dmg_attck_rate, 0, 0];
      } else if (attack_method == 21) {
        const attack_count1 = parseInt(document.getElementById("collei_skill_count1").value);
  ...
  class kachina {
    ...
    async dmg_rate_data() {
      const talent1_check =  document.getElementById("Kachina_talent1");
      if(talent1_check.checked)
      {
        this.talent1_buff = 0.2;
      }
      ...
      if (attack_method == 16) {
        ...
        this.attack_hit_count = 1;
        dmg_deff_rate = parseFloat(data["元素爆発"]["詳細"][0]["数値"][this.parameter[3]]);
    ...
    calculate_basic_dmg(dmg_rate, status) {
      let basicDmg;
      basicDmg = dmg_rate[1] * status[1] + calculate_weapon_basedmg(this.attack_hit_count, status, WeaponConstellations, this.base_dmgbuff);
      return basicDmg;
    }
